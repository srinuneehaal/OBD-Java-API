...
public class VinCommand extends PersistentCommand {
	 ...
	 * Copy ctor.
	 *
	 * @param other a {@link com.github.pires.obd.commands.control.VinCommand}
	 *              object.
	 */
	...
	public static void main(String[] args) {
		...
		String result = "0140:4902013746415257314837344E2:45303035323839";//"0140:490201374641";//"5257314837344E";// "2:45303035323839";

		String vin = performCalculationsLocal(result);

		System.out.println("VIN=" + vin);
	}

	public static String convertHexToStringLocal(String hex) {
		StringBuilder sb = new StringBuilder();
		// 49204c6f7665204a617661 split into two characters 49, 20, 4c...
		for (int i = 0; i < hex.length() - 1; i += 2) {
	...
	public static String performCalculationsLocal(String result) {
		...
		if (result.contains(":")) {// CAN(ISO-15765) protocol.
			workingData = result.replaceAll(".:", "").substring(9);// 9 is xxx490201, xxx is bytes of information to
																	// follow.
			Matcher m = Pattern.compile("[^a-z0-9 ]", Pattern.CASE_INSENSITIVE)
					.matcher(convertHexToStringLocal(workingData));
			if (m.find())
				workingData = result.replaceAll("0:49", "").replaceAll(".:", "");
		} else {// ISO9141-2, KWP2000 Fast and KWP2000 5Kbps (ISO15031) protocols.
			workingData = result.replaceAll("49020.", "");
		}
		String vin = convertHexToStringLocal(workingData).replaceAll("[\u0000-\u001f]", "");
		return vin;
